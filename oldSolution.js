...
const snapraiseCalc = (input) => {
    ...
    // //if we have no operators throw error
    // if(operatorArr.length === 0){
    //     console.log("You need operators")
    //     return null
    // }
    ...
    let total = numberArr[numberArr.length - 1];
...
while(true) {
    ...
    if (usersInput === 'q' || usersInput === 'Q') {
        ...
        const result = snapraiseCalc(usersInput.split(" "))
        ...
        if (result === null){
          ...
          // if the inputs are valid it will run the function
            ...
            console.log("Press Q to exit or start a new expression");
        }
    }
}
